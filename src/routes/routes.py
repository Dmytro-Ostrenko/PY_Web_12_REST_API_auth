...
async def search_contact(contact_query: str = Path(..., min_length=3), db: AsyncSession = Depends(get_db)):
    return await repositories_contacts.search_contact(contact_query, db)
...
async def search_upcoming_birthdays(db: AsyncSession = Depends(get_db)):
    return await repositories_contacts.contacts_upcoming_birthdays(db)
